                    ...
                    <div key={idx} className="flex gap-3 items-start sm:items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                          <p className="text-[11px] text-gray-500 dark:text-slate-400">{t("lookup.variant").replace("{variant}", `${getLocalizedText(item, "color_name") || item.color}, ${item.size}`).replace("{qty}", item.quantity)}</p>
                          {item.is_gift && (
                            <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300">
                  ...
                  <div className="mt-3 pt-3 border-t border-dashed border-gray-300 dark:border-slate-600 space-y-3">
                    ...
                    {order.payment_status === 'Unpaid' && order.status !== 'Cancelled' && (
                      <button
                        onClick={() => handleOpenPaymentModal(order)}
                        disabled={loading}
                        className="w-full mt-3 bg-slate-900 dark:bg-violet-600 hover:bg-slate-800 dark:hover:bg-violet-700 text-white py-2.5 rounded-lg text-sm font-bold transition flex items-center justify-center gap-2"
                      >
                        {loading ? (
                          <i className="fa-solid fa-circle-notch fa-spin"></i>
                        ) : (
                          <>{t("lookup.pay_or_change")}</>
                        )}
                      </button>
                    )}
                    {order.status === 'Delivered' && !order.return_request && (
                      ...
                      <div className="mt-3 space-y-2">
                        {order.return_request ? (
                          <div className="p-3 bg-amber-50/70 dark:bg-amber-950/40 border border-amber-200/70 dark:border-amber-900/50 rounded-xl text-xs space-y-2">
                            ...
                            {/* Returned Items List (same as profile OrderDetailModal) */}
                            {order.return_request.items && order.return_request.items.length > 0 && (
                              <div className="space-y-1.5 border-t border-amber-200/60 dark:border-amber-900/50 pt-2">
                                <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400">{t('order_details.returned_items_title', 'Sản phẩm yêu cầu trả:')}</p>
                                {order.return_request.items.map((ri: any, rIdx: number) => {
                    ...
                    {/* Cancel order button (only for Pending/Confirmed, no active return request) */}
                    {["Pending", "Confirmed"].includes(order.status) && !order.return_request && (
                      <button
                        onClick={() => handleCancelOrder && handleCancelOrder(order.id)}
                        disabled={loading}
                    ...
                    {/* Buy again button (for cancelled / delivered orders) */}
                    {["Cancelled", "Delivered"].includes(order.status) && (
                      <button
                        onClick={() => handleBuyAgain && handleBuyAgain(order)}
                        disabled={loading}
