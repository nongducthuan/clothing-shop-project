            ...
            {["all", "category", "product"].map((scope) => (
              <button
                ...
                className={`flex-1 xl:flex-none px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${
                  applyScope === scope ? 'bg-white dark:bg-slate-600 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-400 bg-transparent'
                }`}
              >
                {scopeLabels[scope]}
              </button>
            ...
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-700/60 border border-slate-200/80 dark:border-slate-600 rounded-xl text-xs focus:ring-2 focus:ring-indigo-500 transition-all outline-none text-slate-800 dark:text-slate-100"
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 rounded-2xl border flex items-center justify-between cursor-pointer transition-all duration-200 ${
                        isSelected ? 'border-indigo-500 bg-indigo-50/50 dark:bg-indigo-900/20 shadow-sm' : 'border-slate-200/80 dark:border-slate-600/60 bg-white dark:bg-slate-700/40 hover:border-slate-300 dark:hover:border-slate-500'
                      }`}
                    >
                      ...
                      <div className="ml-2 flex-shrink-0">
                        <i className={`fa-solid ${isSelected ? 'fa-check-circle text-indigo-500 text-lg' : 'fa-circle text-slate-200 dark:text-slate-600 text-lg'}`}></i>
                      </div>
                    </div>
                  );
                })}
