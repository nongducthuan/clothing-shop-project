    ...
    <>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 h-full">
            ...
            {["all", "category", "product"].map((scope) => (
              <button
                ...
                className={`flex-1 xl:flex-none px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${
                  applyScope === scope ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 bg-transparent'
                }`}
              >
                {scope}
              </button>
        ...
        <div className="min-h-[300px]">
          {/* All Scope */}
          {applyScope === "all" && (
            <div className="flex flex-col items-center justify-center h-64 border-4 border-dashed border-indigo-50 rounded-3xl p-10 text-center">
              <i className="fa-solid fa-store text-4xl text-indigo-200 mb-4"></i>
              <h4 className="font-bold text-gray-800">Store-wide Voucher</h4>
              <p className="text-gray-400 text-xs mt-2">This code will apply to any product in the store.</p>
            </div>
          )}
            ...
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-xs focus:ring-2 focus:ring-indigo-500 transition-all outline-none"
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 rounded-2xl border flex items-center justify-between cursor-pointer transition-all duration-200 ${
                        isSelected ? 'border-indigo-500 bg-indigo-50/50 shadow-sm' : 'border-gray-100 bg-white hover:border-gray-300'
                      }`}
                    >
                      <div className="flex flex-col gap-1 overflow-hidden">
                        <span className={`text-[11px] font-black truncate ${isSelected ? 'text-indigo-700' : 'text-gray-700'}`}>
                          {cat.name}
                        </span>
                        <div className="flex">
                      ...
                      <div className="ml-2 flex-shrink-0">
                        <i className={`fa-solid ${isSelected ? 'fa-check-circle text-indigo-500 text-lg' : 'fa-circle text-gray-100 text-lg'}`}></i>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-xs focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all outline-none"
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>
